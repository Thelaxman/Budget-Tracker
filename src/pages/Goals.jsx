...
const GOAL_TYPES = [
  ...
  { id: 'emergency', label: 'Emergency Fund', icon: '🛡️', color: 'from-amber-500 to-amber-400', bg: 'bg-amber-50', text: 'text-amber-600', bar: 'bg-amber-500' },
  { id: 'vacation', label: 'Vacation', icon: '✈️', color: 'from-purple-500 to-purple-400', bg: 'bg-purple-50', text: 'text-purple-600', bar: 'bg-purple-500' },
  { id: 'home', label: 'Home', icon: '🏠', color: 'from-green-500 to-green-400', bg: 'bg-green-50', text: 'text-green-600', bar: 'bg-green-500' },
  { id: 'other', label: 'Other', icon: '🎯', color: 'from-gray-500 to-gray-400', bg: 'bg-gray-100', text: 'text-gray-600', bar: 'bg-gray-500' },
]
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {goals.length > 0 && (
          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              <div className="text-xs font-medium text-gray-400 mb-1">Total saved</div>
              <div className="text-xl font-bold text-blue-500">{fmt(totalSaved)}</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              <div className="text-xs font-medium text-gray-400 mb-1">Total target</div>
              <div className="text-xl font-bold text-gray-800">{fmt(totalTarget)}</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              <div className="text-xs font-medium text-gray-400 mb-1">Completed</div>
              <div className="text-xl font-bold text-green-500">{completed} / {goals.length}</div>
            </div>
          </div>
        )}
        ...
        <AnimatePresence>
          {showForm && (
            ...
            >
              ...
              >
                <div className="text-sm font-semibold text-gray-700 mb-4">New goal</div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-4">
                  {GOAL_TYPES.map(t => (
                    ...
                    >
                      ...
                      <span className={`text-xs font-medium ${form.type === t.id ? 'text-blue-600' : 'text-gray-500'}`}>
                        {t.label}
                      </span>
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Target ($)</label>
                    <input
                      type="number"
                      value={form.target}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Saved so far ($)</label>
                    <input
                      type="number"
                      value={form.saved}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <div className="text-gray-500 font-medium mb-1">No goals yet</div>
              ...
              {goals.map((g, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm group hover:border-gray-200 transition-all"
                  >
                    {/* Top row */}
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        {done && (
                          <span className="text-xs font-semibold bg-green-50 text-green-500 px-2 py-1 rounded-full">
                            ✓ Done
                          </span>
                        )}
                        <button
                    ...
                    <div className="flex justify-between text-sm mb-4">
                      <div>
                        <span className="font-bold text-gray-900">{fmt(g.saved)}</span>
                        <span className="text-gray-400 text-xs"> saved</span>
                      </div>
                      <div className="text-gray-400 text-xs">
                        of <span className="font-semibold text-gray-600">{fmt(g.target)}</span>
                      </div>
                    </div>
                        ...
                        {addingTo === g.id ? (
                          ...
                          >
                            <input
                              ...
                              min="0"
                              autoFocus
                              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all"
                            />
                            <button
                              onClick={() => addSavings(g)}
                              className={`text-white text-xs font-semibold px-3 py-2 rounded-xl bg-gradient-to-r ${type.color} hover:opacity-90 transition-opacity`}
                            >
                              Add
                            </button>
                            <button
